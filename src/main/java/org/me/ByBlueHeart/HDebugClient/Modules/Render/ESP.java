...
import net.blueheart.hdebug.features.module.modules.render.NormalNameTags;
...
import net.blueheart.hdebug.utils.render.ESPUtils;
import net.blueheart.hdebug.utils.render.RenderUtils;
import net.blueheart.hdebug.utils.render.WorldToScreen;
import net.blueheart.hdebug.utils.render.shader.FramebufferShader;
import net.blueheart.hdebug.utils.render.shader.shaders.GlowShader;
import net.blueheart.hdebug.utils.render.shader.shaders.OutlineShader;
...
public class ESP extends Module {
    ...
    public final ListValue modeValue = new ListValue("Mode", new String[]{"Box", "OtherBox", "WireFrame", "Milins", "2D", "Other2D", "Real2D", "2DBox", "Outline", "ShaderOutline", "ShaderGlow", "Glow"}, "Box");
    ...
    private final IntegerValue slicesValue = new IntegerValue("S", 8, 2, 32);
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        Matrix4f mvMatrix = WorldToScreen.getMatrix(GL11.GL_MODELVIEW_MATRIX);
        Matrix4f projectionMatrix = WorldToScreen.getMatrix(GL11.GL_PROJECTION_MATRIX);
        ...
        for (final Entity entity : mc.theWorld.loadedEntityList) {
            if (entity != null && entity != mc.thePlayer && EntityUtils.isSelected(entity, false)) {
                double x, size, y, boundindY, z, convertedPoints[], xd, posX = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * event.getPartialTicks() - (mc.getRenderManager()).renderPosX;
                double posY = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * event.getPartialTicks() - (mc.getRenderManager()).renderPosY;
                double posZ = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * event.getPartialTicks() - (mc.getRenderManager()).renderPosZ;
                final EntityLivingBase entityLiving = (EntityLivingBase) entity;
                Color color = getColor(entityLiving);
                switch (mode.toLowerCase()) {
                    case "box":
                    case "milins": {
                        ESPUtils.cylinder(entity, posX, posY, posZ, this.rangeValue.get().intValue(), this.slicesValue.get().intValue());
                        ESPUtils.shadow(entity, posX, posY, posZ, this.rangeValue.get().intValue(), this.slicesValue.get().intValue());
                        break;
                    }
                    case "otherbox": {
                    ...
                    case "2d": {
                        ...
                        final double posXa = entityLiving.lastTickPosX + (entityLiving.posX - entityLiving.lastTickPosX) * timer.renderPartialTicks - renderManager.renderPosX;
                        final double posYa = entityLiving.lastTickPosY + (entityLiving.posY - entityLiving.lastTickPosY) * timer.renderPartialTicks - renderManager.renderPosY;
                        final double posZa = entityLiving.lastTickPosZ + (entityLiving.posZ - entityLiving.lastTickPosZ) * timer.renderPartialTicks - renderManager.renderPosZ;

                        RenderUtils.draw2D(entityLiving, posXa, posYa, posZa, getColor(entityLiving).getRGB(), Color.BLACK.getRGB());
                        break;
                    }
                    case "2dbox": {
                        GL11.glPushMatrix();
                        ...
                        size = 0.5D;
                        boundindY = (entity.getEntityBoundingBox()).maxY - (entity.getEntityBoundingBox()).minY + 0.2D;
                    ...
                    case "other2d": {
                        ...
                        y = entityLiving.lastTickPosY + (entityLiving.posY - entityLiving.lastTickPosY) * pTicks - mc.getRenderManager().viewerPosY;
                        ...
                        convertedPoints = convertTo2D(x, y, z);
                        xd = Math.abs(convertTo2D(x, y + 1.0D, z, entityLiving)[1] - convertTo2D(x, y, z, entityLiving)[1]);
    ...
    private void renderOther2dEntity(EntityLivingBase ent) {
        ...
        if ((renderPositions[3] > 0.0D) || (renderPositions[3] <= 1.0D)) {
            ...
            if (ent instanceof EntityPlayer) {
                if (!HDebug.moduleManager.getModule(NameTags.class).getState() && (names.get()) || !HDebug.moduleManager.getModule(NormalNameTags.class).getState() && (names.get())) {
                    GlStateManager.pushMatrix();
