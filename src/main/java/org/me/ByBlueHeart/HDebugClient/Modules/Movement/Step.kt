...
import org.me.ByBlueHeart.HDebugClient.Modules.Player.Phase
...
class Step : Module() {
    ...
    fun onStep(event: StepEvent) {
        ...
        if (HDebug.moduleManager[Phase::class.java]!!.state) {
            event.stepHeight = 0F
            return
        }

        // Some fly modes should disable step
        val fly = HDebug.moduleManager[Flight::class.java] as Flight
        if (fly.state) {
