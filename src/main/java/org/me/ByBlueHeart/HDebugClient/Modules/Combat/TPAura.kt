...
import net.blueheart.hdebug.utils.PathUtils
import net.blueheart.hdebug.utils.RotationUtils
import net.blueheart.hdebug.utils.astar.Astar
import net.blueheart.hdebug.utils.astar.NaiveAstarFlyNode
import net.blueheart.hdebug.utils.astar.NaiveAstarGroundNode
import net.blueheart.hdebug.utils.astar.NaiveAstarNode
import net.blueheart.hdebug.utils.block.BlockUtils
import net.blueheart.hdebug.utils.block.BlockUtils.bBoxIntersectsBlock
...
@ModuleInfo(name = "TPAura", description = "Teleport Kill Player.", category = ModuleCategory.COMBAT)
class TPAura : Module() {

    // PPS:packets per sec
    private val pPS = IntegerValue("PPS", 18, 0, 10)

    private val rangeValue = FloatValue("Range", 20f, 1f, 200f)
    ...
    //queue
    private val TPAuraQueue = mutableListOf<Packet<INetHandlerPlayServer>>()

    /**
     * Enable TPAura module
     */
        ...
        if (path.size != 0)
            ...
            for (vector3d in path) {
                if (isNodeValid(vector3d))
                    TPAuraQueue += C03PacketPlayer.C04PacketPlayerPosition(
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        for (i in TPAuraQueue) {
            if (i is C03PacketPlayer.C04PacketPlayerPosition) {
                ...
                val blue = max(min((index.toDouble() / TPAuraQueue.size * 255.0).toInt(), 255), 0)
                index++
    ...
    fun onTick(event: TickEvent) {
        ...
        while (TPAuraQueue.size < pPS.get() * 0.3) {
        ...
        val begin = TPAuraQueue.first()
        if (!scheduled && pulse.get() && rescheduleOnMove.get() && packets > TPAuraQueue.size && TPAuraQueue.size > 0)
        ...
        while (packets > 0 && TPAuraQueue.size > 0 && (!pulse.get() || packets > TPAuraQueue.size)) {
            val first = TPAuraQueue.first()
    ...
    private fun pathFindToCoord(
        ...
        if (lowerCasePathfindString == "normal") {
            val diffX = toX - fromX
            val diffY = toY - fromY
            val diffZ = toZ - fromZ
    ...
    private fun isNodeValid(vec3: Vector3d): Boolean {
        if (TPAuraQueue.size == 0) return true

        val lastQueuePacket: C03PacketPlayer.C04PacketPlayerPosition =
                (when {
                    TPAuraQueue.last() is C03PacketPlayer.C04PacketPlayerPosition -> TPAuraQueue.last()
                    TPAuraQueue[TPAuraQueue.size - 2] is C03PacketPlayer.C04PacketPlayerPosition -> TPAuraQueue[TPAuraQueue.size - 2]
                    else -> null
                }
    ...
    private fun tpBack() {
        val me = mc.thePlayer
        // TP back
        TPAuraQueue += attackPacketsCache
    ...
    private fun runAttack(): Boolean {
        ...
        if (openInventory)
            TPAuraQueue.add(C0DPacketCloseWindow())
        ...
        if (mc.thePlayer.getDistanceToEntity(target) < stopAtDistance.get() || path != null) {
            if (path != null)
                for (vec3 in path) {
                    if (isNodeValid(vec3)) {
                        TPAuraQueue.add(C03PacketPlayer.C04PacketPlayerPosition(vec3.x, vec3.y, vec3.z, true))
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        TPAuraQueue.add(C02PacketUseEntity(entity, C02PacketUseEntity.Action.ATTACK))
    ...
    override val tag: String?
        get() = targetModeValue.get() + ' ' + pathFindingMode.get() + ' ' + TPAuraQueue.size.toString()
}