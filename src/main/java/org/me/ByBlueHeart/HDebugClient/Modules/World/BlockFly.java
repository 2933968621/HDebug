...
import net.blueheart.hdebug.features.module.modules.render.BlockOverlay;
import net.blueheart.hdebug.ui.font.Fonts;
import net.blueheart.hdebug.utils.*;
...
@ModuleInfo(name = "BlockFly", description = "Automatically places blocks beneath your feet.", category = ModuleCategory.WORLD)
public class BlockFly extends Module {
    ...
    // Turn Speed
    private final IntegerValue maxTurnSpeedValue = new IntegerValue("MaxTurnSpeed", 180, 1, 180) {
        @Override
        protected void onChanged(final Integer oldValue, final Integer newValue) {
            int v = minTurnSpeedValue.get();
            if (v > newValue) set(v);
            if (getMaximum() < newValue) {
                set(getMaximum());
            } else if (getMinimum() > newValue) {
    ...
    private final IntegerValue minTurnSpeedValue = new IntegerValue("MinTurnSpeed", 180, 1, 180) {
        @Override
        protected void onChanged(final Integer oldValue, final Integer newValue) {
            int v = maxTurnSpeedValue.get();
            if (v < newValue) set(v);
            if (getMaximum() < newValue) {
                set(getMaximum());
            } else if (getMinimum() > newValue) {
