...
@ModuleInfo(name = "AntiBot", description = "Prevents Aura from attacking AntiCheat bots.", category = ModuleCategory.MISC)
public class AntiBot extends Module {
    ...
    private final BoolValue HypixelKiller = new BoolValue("HypixelKiller", true);
    private final BoolValue Hypixel = new BoolValue("Hypixel", true);
    private final BoolValue HuaYuTing = new BoolValue("HuaYuTing", false);
    private final BoolValue HuaYuTing2 = new BoolValue("HuaYuTing4v4", false);
    ...
    public void onUpdate(UpdateEvent e) {
        if (HypixelKiller.get()) {
            if (!removed.isEmpty()) {
                if (lastRemoved.delay(1000)) {
                    if (removed.size() == 1) {
                        Logger.printinfo("\247a"+removed.size() + " bots is remove");
                    } else {
                        Logger.printinfo("\247a"+removed.size() + " bots is remove");
                    }
                    lastRemoved.reset();
    ...
    public static boolean isBot(final EntityLivingBase entity) {
        ...
        final AntiBot antiBot = (AntiBot) HDebug.moduleManager.getModule(AntiBot.class);

        if (antiBot == null || !antiBot.getState())
            return false;

        if ((antiBot.Hypixel.get().booleanValue() && entity.getName().contains("§")) || (entity.hasCustomName() && entity.getCustomNameTag().contains(entity.getName())))
            return true;

        if (((Boolean)antiBot.HuaYuTing.get()).booleanValue() && entity.ticksExisted < 120)
            return true;

        if (((Boolean)antiBot.HuaYuTing2.get()).booleanValue()) {
            EntityPlayer player = (EntityPlayer) entity;
            if (player.inventory.armorInventory[3] == null && player.inventory.armorInventory[1] == null)
                return true;
        }
