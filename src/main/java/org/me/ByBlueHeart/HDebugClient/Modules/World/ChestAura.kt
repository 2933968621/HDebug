...
import net.blueheart.hdebug.utils.extensions.getVec
import net.blueheart.hdebug.utils.timer.MSTimer
import net.blueheart.hdebug.value.BlockValue
...
object ChestAura : Module() {
    ...
    private val onMiddleClickValue = BoolValue("MiddleClick", false)
    ...
    private var clickedBlocksCleared = false;
    ...
    fun onMotion(event: MotionEvent) {
        if (HDebug.moduleManager[Blink::class.java]!!.state)
            return
        val takeItems = Mouse.isButtonDown(2) || !onMiddleClickValue.get()
        when (event.eventState) {
            ...
            EventState.POST -> if (takeItems && currentBlock != null && timer.hasTimePassed(delayValue.get().toLong())) {
                if (mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.heldItem, currentBlock,
                    ...
                    if (!onMiddleClickValue.get()) {
                        clickedBlocks.add(currentBlock!!)
                        clickedBlocksCleared = false;
                    } else if (!clickedBlocksCleared) {
                        clickedBlocks.clear()
                        clickedBlocksCleared = true;
                    }
                    currentBlock = null
                    timer.reset()
                }
