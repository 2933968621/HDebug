...
import net.blueheart.hdebug.utils.block.BlockUtils.searchBlocks
...
class Nuker : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        // Block hit delay
        if (blockHitDelay > 0 && !HDebug.moduleManager[SpeedMine::class.java]!!.state) {
            blockHitDelay--
            return
        }
        ...
        if (mc.playerController.isNotCreative) {
            ...
            do{
                ...
                // Call auto tool
                val autoTool = HDebug.moduleManager.getModule(AutoTool::class.java) as AutoTool
                if (autoTool.state)
