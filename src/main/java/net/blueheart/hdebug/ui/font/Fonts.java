...
public class Fonts {
    ...
    public static void loadFonts() {
        ...
        try {
            ...
            final File fontsFile = new File(HDebug.fileManager.fontsDir, "fonts.json");
    ...
    private static void downloadFonts() {
        try {
            final File outputFile = new File(HDebug.fileManager.fontsDir, "fonts.zip");

            if(!outputFile.exists()) {
                ClientUtils.getLogger().info("Downloading fonts...");
                HttpUtils.download("https://cpnh8n.coding-pages.com/fonts.zip", outputFile);
                ClientUtils.getLogger().info("Extract fonts...");
                extractZip(outputFile.getPath(), HDebug.fileManager.fontsDir.getPath());
            }
        }catch(IOException e) {
            e.printStackTrace();
        }
    ...
    private static Font getFont(final String fontName, final int size) {
        try {
            final InputStream inputStream = new FileInputStream(new File(HDebug.fileManager.fontsDir, fontName));
