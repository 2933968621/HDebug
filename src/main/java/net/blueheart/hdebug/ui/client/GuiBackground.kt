...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(HDebug.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(HDebug.fileManager.backgroundFile))
                    HDebug.background = ResourceLocation(HDebug.CLIENT_NAME.toLowerCase() + "/background.png")
                    mc.textureManager.loadTexture(HDebug.background, DynamicTexture(image))
                } catch (e: Exception) {
                    e.printStackTrace()
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        drawBackground(0)
        Fonts.fontBold180.drawCenteredString("Background", this.width / 2F, height / 8F + 5F, 0x00ffff, true)
