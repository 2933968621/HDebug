...
import net.blueheart.hdebug.features.command.shortcuts.Shortcut
...
class ShortcutsConfig(file: File) : FileConfig(file) {
    ...
    override fun loadConfig() {
        ...
        for (shortcutJson in jsonElement) {
            ...
            for (scriptCommand in scriptJson) {
                ...
                val command = HDebug.commandManager.getCommand(commandName) ?: continue

                script.add(Pair(command, arguments.map { it.asString }.toTypedArray()))
            }

            HDebug.commandManager.registerCommand(Shortcut(name, script))
        }
    }
    ...
    override fun saveConfig() {
        ...
        for (command in HDebug.commandManager.commands) {
