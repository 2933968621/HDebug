...
class BindCommand : Command("bind", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        if (args.size > 2) {
            // Get module by name
            val module = HDebug.moduleManager.getModule(args[1])
            ...
            HDebug.hud.addNotification(Notification("Bound ${module.name} to ${Keyboard.getKeyName(key)}"))
            playEdit()
            return
        }
    ...
    override fun tabComplete(args: Array<String>): List<String> {
        ...
        return when (args.size) {
            1 -> HDebug.moduleManager.modules
                    .map { it.name }
