...
import net.blueheart.hdebug.utils.SettingsUtils
...
class LocalAutoSettingsCommand : Command("localautosettings", arrayOf("localsetting", "localsettings", "localconfig")) {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val scriptFile = File(HDebug.fileManager.settingsDir, args[2])
                        ...
                        if (scriptFile.exists()) {
                            try {
                                ...
                                HDebug.hud.addNotification(Notification("Updated Settings"))
                                playEdit()
                            } catch (e: IOException) {
                                e.printStackTrace()
                            }
    ...
    private fun getLocalSettings(): Array<File>? = HDebug.fileManager.settingsDir.listFiles()
}